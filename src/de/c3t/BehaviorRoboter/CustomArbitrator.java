/*this file is a copy of  lejos.robotics.subsumptionArbitrator. 
 * This class had the bug that supress() never was called, 
 * when a behavior with a higher priority sends takeControl() == false
 **/
...
/**
 * Arbitrator controls which Behavior object will become active in a behavior control system. Make
 * sure to call start() after the Arbitrator is instantiated.<br>
 ...
 * 3. When the action() method exits, call action() on the Behavior of highest priority. <br>
 * The Arbitrator assumes that a Behavior is no longer active when action() exits, <br>
 * therefore it will only call suppress() on the Behavior whose action() method is running. <br>
...
public class CustomArbitrator {
	...
	/**
	 * Allocates an Arbitrator object and initializes it with an array of Behavior objects. The index
	 * of a behavior in this array is its priority level, so the behavior of the largest index has the
	 * highest the priority level. The behaviors in an Arbitrator can not be changed once the
	 ...
	 *          if <B>true</B>, the <B>start()</B> method returns when no Behavior is active.
	 */
	...
	/**
	 * Finds the highest priority behavior that returns <B>true </B> to takeControl(); If this
	 * priority is higher than the active behavior, it calls active.suppress(). If there is no active
	 * behavior, calls suppress() on the most recently acrive behavior.
	 */
